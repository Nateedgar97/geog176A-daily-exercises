...
```{r}
q1=covid %>% filter(date==max(date)) %>% arrange(-cases) %>% top_n(5)
q1
```
...
```{r}
q2=covid %>% filter(date==max(date)) %>% group_by(state) %>% summarize(totalCases = sum(cases, na.rm = TRUE)) %>% arrange(totalCases) %>% arrange(-totalCases) %>% top_n(5)
q2
```
...
```{r}
q3=covid %>%
filter(date==max(date)) %>% mutate(ratio=100*(deaths/cases)) %>%
arrange(-cases)%>%
slice_max(ratio, n=5)
q3
```
...
ungroup() %>%
mutate(ratio=100*(totDeaths/totCases)) %>% slice_max(ratio,n=5)
q4
```
